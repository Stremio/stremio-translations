...
function writeDataClass() {
  const defaultTranslation = JSON.parse(
    fs.readFileSync(path.join(process.cwd(), "en-US.json")),
  );
  let stringClass = `package ${packageName}\n\n`;
  stringClass += "interface Strings{\n";
  const dedup = deduplicate(defaultTranslation);
  Object.keys(dedup).forEach(
    (key) => (stringClass += `  val ${key}: String\n`),
  );
  stringClass += "  val entries: Map<String, String>\n";
  stringClass += "}";
  fs.writeFileSync(
    path.join(process.cwd(), rootPath, "Strings.kt"),
    stringClass,
  );
}
...
function writeLocales() {
  ...
  files.forEach(
    (file) =>
      (stringClass += `  const val ${toLocaleName(file)} = "${fileName(file)}"\n`),
  );
  stringClass += "}";
  fs.writeFileSync(
    path.join(process.cwd(), rootPath, "Locales.kt"),
    stringClass,
  );
}
...
function writeTranslations() {
  ...
  files.forEach(
    (file) =>
      (stringClass += `import com.stremio.translations.${toClassName(file)}\n`),
  );
  stringClass += "\n";
  stringClass += "val stremioTranslations = mapOf(\n";
  stringClass += files
    .map((file) => `  Locales.${toLocaleName(file)} to ${toClassName(file)}`)
    .join(",\n");
  stringClass += ",\n)";
  fs.writeFileSync(
    path.join(process.cwd(), rootPath, "Translations.kt"),
    stringClass,
  );
}
...
function writeStrings() {
  files.forEach((file) => {
    ...
    fs.writeFileSync(
      path.join(process.cwd(), stringsPath, `${langClassName}.kt`),
      stringClass,
    );
  });
}
...
function toClassName(file) {
  return file.replace(
    /(\w\w)-(\w\w)\.json/,
    (_, p1, p2) => `${p1.charAt(0).toUpperCase() + p1.slice(1)}${p2}Strings`,
  );
}
