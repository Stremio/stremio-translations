...
function writeLocales() {
  ...
  resource += "<resources>\n";
  resource +=
    '    <string-array name="interface_locales" translatable="false">\n';
  files.forEach(
    (file) => (resource += `        <item>${file.split(".").shift()}</item>\n`),
  );
  resource += "    </string-array>\n";
  resource += "</resources>";
  fs.mkdirSync(path.join(process.cwd(), rootPath, "values"));
  fs.writeFileSync(
    path.join(process.cwd(), rootPath, "values", "array.xml"),
    resource,
  );
}
...
function writeTranslations() {
  files.forEach((file) => {
    ...
    xml += "</resources>";
    fs.mkdirSync(path.join(process.cwd(), rootPath, langFolder), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(process.cwd(), rootPath, langFolder, "strings.xml"),
      xml,
    );
